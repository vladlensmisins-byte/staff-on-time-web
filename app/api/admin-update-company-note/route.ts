...
export async function POST(request: NextRequest) {
  try {
    ...
    if (fetchError) {
      console.error("Company note fetch failed:", fetchError.message);
    ...
    const notes = serializeAdminComments(nextComments);
    const { error } = await supabase.from("companies").update({ notes }).eq("id", id);

    if (error) {
      console.error("Company note update failed:", error.message);
    ...
    console.error("POST /api/admin-update-company-note error:", err);
